...
module.exports = DeployPlugin.extend({
  ...
  activate: function() {
    ...
    return this.fetchRevisions().then(data => {
      let activateTask = new ActivateTask({ revisions: data.revisions, region, bucket, prefix, suffix, revisionKey, key, log });
  ...
  _fetchRevisions(as) {
    ...
    let revisionsTask = new RevisionsTask({ region, bucket, prefix, suffix, key });
